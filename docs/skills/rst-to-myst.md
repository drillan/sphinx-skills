...
既存の reStructuredText (`.rst`) Sphinx プロジェクトを MyST Markdown (`.md`) に移行するスキル。`rst-to-myst` ツールを ad-hoc 実行 (依存に追加せず単発実行) し、変換結果を 3 段階で検証してから原本を削除する。**実コンテンツを持つ既存プロジェクト** が対象であり、`sphinx-quickstart` 生成の最小 `index.rst` は対象外。
...
`rst-to-myst` パッケージは依存に含めず単発実行する。
...
単一ファイル または `docs/` 配下全体。原本 `.rst` のサイズを記録 (検証で参照)。

### 2. ad-hoc 変換実行

検出した PM に対応するコマンドを実行する。
...
いずれか不成立の場合は原本 `.rst` を残し、生成された不完全な `.md` を削除し、検証失敗の理由を含む例外を送出する (デフォルト値での継続禁止)。
