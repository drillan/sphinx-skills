...
既存の reStructuredText (`.rst`) Sphinx プロジェクトを MyST Markdown (`.md`) に移行するスキルです。`rst-to-myst` ツールを ad-hoc 実行 (依存に追加せず単発実行) し、変換結果を 3 段階で検証してから原本を削除します。**実コンテンツを持つ既存プロジェクト** が対象であり、`sphinx-quickstart` 生成の最小 `index.rst` は対象外です。
