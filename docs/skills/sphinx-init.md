...
Sphinx ドキュメントプロジェクトを新規作成する初期化スキルです。プロジェクトのパッケージマネージャ (uv 推奨、poetry / pipenv / plain venv も対応) を自動検出して依存追加を行い、RST または MyST (Markdown) 記法を選択できます。`livehtml` と `latexpdfja` ターゲットを含むカスタム Makefile も生成します。
...
検出した PM のコマンドが PATH に無ければ例外を送出します。デフォルト値による継続処理は行いません。
...
8. `conf.py` 反映は `sphinx-config` に委譲
...
`linkify` 選択時は `linkify-it-py` を docs 依存に自動追加します。
