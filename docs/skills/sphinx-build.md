...
`docs/Makefile` 経由で Sphinx ドキュメントをビルドするスキル。HTML、クリーン再ビルド、日本語 PDF (`latexpdfja`)、EPUB、リンクチェック、ライブリロード開発サーバ (`livehtml`) を統一インターフェースで扱う。
...
1. `docs/Makefile` が存在すること — 不在なら明示的エラー伝播 + `sphinx-init` 誘導 (Makefile 自動生成等の暗黙処理は行わない)
2. 実行 CWD はプロジェクトルート (`pyproject.toml` のあるディレクトリ) 前提。サブディレクトリから呼ばれた場合はプロジェクトルートへ移動してから実行
3. PM 検出 (uv / poetry / pipenv / plain venv)

## ターゲットマッピング

uv 検出時のコマンド例。他 PM では `uv run` 部分を `poetry run` / `pipenv run` / venv 有効化に置換する。
...
ビルド失敗時、出力を解析して以下のパターンに該当する場合は修正候補を提示する。
